...
Perf_2_2_Ander =  table2array(importPressAndForce(datafilepath2 + "Perforation_2_2_0-4.txt", 9, inf));
...
RMSE_CFD(1) = sqrt(sum((bestfit(0.99,0.92,x1) - bestfit(s(1,1),s(1,2),x1)).^2)/length(x1));
RMSE_CFD(2) = sqrt(sum((bestfit(0.91,0.67,x1) - bestfit(s(2,1),s(2,2),x1)) .^2)/length(x1));
RMSE_CFD(3) = sqrt(sum((bestfit(0.68,0.62,x1) - bestfit(s(3,1),s(3,2),x1)).^2)/length(x1));

%% R-Squared Calculation
R_SQ_CFD(1) = R_SQ(bestfit(0.99,0.92,x1), bestfit(s(1,1),s(1,2),x1));
R_SQ_CFD(2) = R_SQ(bestfit(0.91,0.67,x1), bestfit(s(2,1),s(2,2),x1));
R_SQ_CFD(3) = R_SQ(bestfit(0.68,0.62,x1), bestfit(s(3,1),s(3,2),x1));
...
hold on
fplot(@(x) 0.99*tanh(0.92*x), [1 6], 'g');
%fplot(@(x) s(1,1)*tanh(s(1,2)*x), [1 6], 'm');
title("Perforation #1 Relative Force vs. KC number")
xlabel("KC")
ylabel("Perforation Force/Reference Force")
ylim ([0.3 1.1])
legend('CFD Retest', 'Original CFD Line of Best Fit')
functionText1 = sprintf('RMSE = %.4f', RMSE_CFD(1));
functionText2 = sprintf('R^2 = %.4f', R_SQ_CFD(1));
...
hold on
fplot(@(x) 0.91*tanh(0.67*x), [1 6], 'g');
%fplot(@(x) s(2,1)*tanh(s(2,2)*x), [1 6], 'm');
title("Perforation #2 Relative Force vs. KC number")
xlabel("KC")
ylabel("Perforation Force/Reference Force")
ylim ([0.3 1.1])
legend('CFD Retest', 'Original CFD Line of Best Fit')
functionText1 = sprintf('RMSE = %.4f', RMSE_CFD(2));
functionText2 = sprintf('R^2 = %.4f', R_SQ_CFD(2));
text(1.5, .4, functionText1, 'FontSize', 10, 'HorizontalAlignment', 'left');
text(1.5, .36, functionText2, 'FontSize', 10, 'HorizontalAlignment', 'left');
...
hold on
fplot(@(x) 0.68*tanh(0.62*x), [1 6], 'g');
%fplot(@(x) s(3,1)*tanh(s(3,2)*x), [1 6], 'm');
title("Perforation #3 Relative Force vs. KC number")
xlabel("KC")
ylabel("Perforation Force/Reference Force")
ylim ([0.3 1.1])
legend('CFD Retest', 'Original CFD Line of Best Fit')
functionText1 = sprintf('RMSE = %.4f', RMSE_CFD(3));
functionText2 = sprintf('R^2 = %.4f', R_SQ_CFD(3));
